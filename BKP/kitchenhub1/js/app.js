...
function syncWishlistUI(){
  document.querySelectorAll('[data-wid]').forEach(btn=>{
    const id=parseInt(btn.dataset.wid),on=wishlist.includes(id);
...
function toast(msg,type='info',dur=3000){
  ...
  el.innerHTML=`<span class="toast-ico">${type==='success'?'✓':type==='error'?'✕':'ℹ'}</span><span>${msg}</span>`;
  wrap.appendChild(el);
...
function renderCard(p){
  const fav=wishlist.includes(p.id),save=p.oldPrice-p.price;
  return `<div class="pcard" data-id="${p.id}">
    <div class="pcard-img" onclick="openQV(${p.id})">
      ...
      <span class="emoji-fb" style="display:none;align-items:center;justify-content:center;width:100%;height:100%;font-size:3.4rem">${p.emoji}</span>
    </div>
...
function openQV(id){
  ...
  ov.querySelector('.modal').innerHTML=`
    ...
    <div class="modal-grid">
      <div>
        <div class="gal-main" style="font-size:6rem">
          <img src="${p.img.replace('w=400&h=400','w=600&h=600')}" alt="${p.name}" onerror="this.style.display='none'">
        </div>
        <div class="gal-thumbs">
          ${IMG[p.cat].slice(0,4).map((src,i)=>`<div class="g-thumb ${i===0?'active':''}" onclick="this.parentNode.querySelectorAll('.g-thumb').forEach(t=>t.classList.remove('active'));this.classList.add('active');this.closest('.modal').querySelector('.gal-main img').src='${src.replace('w=400&h=400','w=600&h=600')}'"><img src="${src.replace('w=400&h=400','w=130&h=130')}" alt="" onerror="this.outerHTML='<span>${p.emoji}</span>'"></div>`).join('')}
        </div>
      </div>
      <div>
        ...
        <div class="pcard-stars" style="margin-bottom:12px">
          ...
          ${p.isBest?'<span class="badge badge-pri" style="font-size:.64rem;margin-left:4px">BESTSELLER</span>':''}
        </div>
        ...
        ${p.colors.length?`<div class="var-label">Color</div><div class="var-row" style="margin-bottom:14px">${p.colors.map((c,i)=>`<button class="var-btn ${i===0?'active':''}" onclick="this.parentNode.querySelectorAll('.var-btn').forEach(b=>b.classList.remove('active'));this.classList.add('active')">${c}</button>`).join('')}</div>`:''}
        ${p.sizes.length?`<div class="var-label">Size</div><div class="var-row" style="margin-bottom:14px">${p.sizes.map((s,i)=>`<button class="var-btn ${i===0?'active':''}" onclick="this.parentNode.querySelectorAll('.var-btn').forEach(b=>b.classList.remove('active'));this.classList.add('active')">${s}</button>`).join('')}</div>`:''}
        <div class="qty-row">
          <div class="qty-ctrl">
        ...
        <a href="product.html?id=${p.id}" style="font-size:.80rem;color:var(--primary);font-weight:600;display:block;margin-top:8px">View Full Details →</a>
      </div>
    </div>`;
...
function initSearch(){
  document.querySelectorAll('.nav-search').forEach(wrap=>{
    const inp=wrap.querySelector('input'),drop=wrap.querySelector('.search-drop');if(!inp||!drop)return;
    inp.addEventListener('input',()=>{
      const v=inp.value.toLowerCase().trim();if(!v){drop.classList.remove('open');return;}
      const m=products.filter(p=>p.name.toLowerCase().includes(v)||p.brand.toLowerCase().includes(v)).slice(0,6);
      if(!m.length){drop.classList.remove('open');return;}
      drop.innerHTML=m.map(p=>`<div class="search-item" onclick="openQV(${p.id})"><span>${p.emoji}</span><span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.name}</span><span class="search-item-price">KSh ${p.price.toLocaleString()}</span></div>`).join('');
...
function setThemeIcons(t){document.querySelectorAll('.theme-btn').forEach(b=>{b.innerHTML=t==='dark'?'☀️':'🌙';b.title=t==='dark'?'Light mode':'Dark mode';});}
...
/* ── SCROLL REVEAL ──────────────────────────────────── */
function initReveal(){
  const els=document.querySelectorAll('.pcard,.cat-card,.feat-card,.testi-card');
  const obs=new IntersectionObserver(entries=>{
    entries.forEach((e,i)=>{
      if(!e.isIntersecting)return;
  ...
  els.forEach(el=>{el.style.opacity='0';el.style.transform='translateY(16px)';el.style.transition='opacity .5s ease,transform .5s ease';obs.observe(el);});
}
...
function initHScroll(){
  document.querySelectorAll('.pscroll-wrap').forEach(wrap=>{
    ...
    if(prev)prev.addEventListener('click',()=>track.scrollBy({left:-(track.offsetWidth*.75),behavior:'smooth'}));
    if(next)next.addEventListener('click',()=>track.scrollBy({left:track.offsetWidth*.75,behavior:'smooth'}));
  });
}
...
function renderCartItems(){
  ...
  el.innerHTML=cart.map(item=>{
    ...
    return`<div class="cart-item">
      <div class="ci-img"><img src="${p.img}" alt="${p.name}" onerror="this.style.display='none'">${p.emoji}</div>
      <div class="ci-info">
...
document.addEventListener('DOMContentLoaded',()=>{
  initNavbar();initSidebar();initSearch();initTheme();initCounters();initCountdown();initHScroll();
